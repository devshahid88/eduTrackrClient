...
const Dashboard = () => {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        if (studentResponse.data.success) {
          ...
          const [assignmentRes, scheduleRes, notifRes] = await Promise.all([
            axios.get(`/api/assignments/department/${student.departmentId}`, { headers: { Authorization: `Bearer ${accessToken}` } }),
            axios.get(`/api/schedules/department/${student.departmentId}`, { headers: { Authorization: `Bearer ${accessToken}` } }),
  ...
  const stats = useMemo(() => {
    ...
    const gradedSubmissions = assignments
      ...
      .filter(s => s.studentId === studentId && s.grade !== undefined && s.grade !== null);
    
    const avgGrade = gradedSubmissions.length > 0 
      ? (gradedSubmissions.reduce((acc, curr) => acc + curr.grade, 0) / gradedSubmissions.length).toFixed(1)
      : '0.0';

    return { activeCourses, pendingAssignments, avgGrade };
  }, [assignments, schedules, studentId]);

  const chartData = useMemo(() => {
    // Show last 5 graded assignments
    return assignments
      .filter(a => a.submissions?.some((s: any) => s.studentId === studentId && s.grade !== undefined))
      .map(a => {
        const mySub = a.submissions.find((s: any) => s.studentId === studentId);
        return {
  ...
  const recentActivity = useMemo(() => {
    ...
    // Graded submissions with feedback as activity
    assignments.forEach(a => {
      ...
      if (mySub && mySub.grade !== undefined) {
        activities.push({
          ...
          description: `You received ${mySub.grade}/${a.maxMarks} for ${a.title}`,
          time: new Date(mySub.submittedAt), // Roughly
    ...
    // New assignments
    assignments.filter(a => !a.submissions?.some((s:any) => s.studentId === studentId)).forEach(a => {
      activities.push({
        id: `new-a-${a._id}`,
        type: 'assignment',
        title: 'New Assignment',
        description: `${a.title} is now available for submission.`,
  ...
  }, [assignments, notifications, studentId]);
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col justify-center items-center h-[70vh] gap-4">
        ...
        <p className="text-gray-500 animate-pulse font-medium text-sm">Synchronizing your dashboard...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-gray-900 tracking-tight">
            Hi, {studentData?.firstname || 'Student'}! 🎓
          </h1>
          <p className="text-gray-500 font-medium mt-1">
            Let's see what you need to focus on today.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        />
        <StatCard 
          icon={<MdAssignment className="text-amber-600" />} 
          label="Pending Work" 
        ...
        <StatCard 
          ...
          label="Latest GPA" 
          value={stats.avgGrade === '0.0' ? '-' : stats.avgGrade} 
          trend="Academic performance"
          color="emerald"
        />
        <StatCard 
          icon={<MdCalendarToday className="text-indigo-600" />} 
          label="Today's Classes" 
          value={schedules.filter((s:any) => s.day === new Date().toLocaleDateString("en-US", { weekday: "long" })).length} 
          trend="Don't be late!"
          color="indigo"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h3 className="text-xl font-black text-gray-900">Academic Progress</h3>
                <p className="text-sm text-gray-500 font-medium">Your recent assignment scores</p>
              </div>
            ...
            <div className="h-72 w-full">
              {chartData.length > 0 ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-gray-400 gap-2 border-2 border-dashed border-gray-100 rounded-[2rem]">
                   ...
                   <p className="text-sm font-medium">Complete assignments to track progress</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             ...
             <div className="bg-gray-900 p-8 rounded-[2rem] shadow-xl text-white">
                <h3 className="text-xl font-black mb-6">Quick Hub</h3>
                <div className="grid grid-cols-2 gap-4">
                   ...
                   >
                      <MdAssignment className="text-2xl mb-2 text-blue-400 group-hover:scale-110 transition-transform" />
                   ...
                   <button 
                     ...
                     className="flex flex-col items-center justify-center p-4 bg-white/5 hover:bg-white/10 rounded-2xl transition-all group"
                   >
                      <MdTrendingUp className="text-2xl mb-2 text-emerald-400 group-hover:scale-110 transition-transform" />
                      <span className="text-[10px] font-bold uppercase tracking-widest">Grades</span>
                   </button>
                   <button 
                     onClick={() => navigate('/student/calendar')}
                     className="flex flex-col items-center justify-center p-4 bg-white/5 hover:bg-white/10 rounded-2xl transition-all group"
                   >
                      <MdCalendarToday className="text-2xl mb-2 text-indigo-400 group-hover:scale-110 transition-transform" />
                   ...
                   <button 
                     ...
                     className="flex flex-col items-center justify-center p-4 bg-white/5 hover:bg-white/10 rounded-2xl transition-all group"
                   >
                      <MdSchool className="text-2xl mb-2 text-amber-400 group-hover:scale-110 transition-transform" />
                      <span className="text-[10px] font-bold uppercase tracking-widest">Lib</span>
                   </button>
                </div>
             </div>
             ...
             <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden relative">
                <div className="absolute top-0 right-0 p-4">
                   <MdCalendarToday className="text-blue-50 text-6xl rotate-12" />
                </div>
                ...
                <div className="space-y-4 relative z-10 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                   {schedules
                    ...
                    .map((s:any) => (
                      <div key={s._id} className="flex flex-col p-3 bg-blue-50/50 rounded-xl border border-blue-50/50">
                         <span className="text-xs font-black text-blue-900">{s.courseId?.name}</span>
                         <span className="text-[10px] font-bold text-blue-500 mt-1">{s.startTime} - {s.endTime}</span>
                      </div>
                   ))}
                   {schedules.filter((s:any) => s.day === new Date().toLocaleDateString("en-US", { weekday: "long" })).length === 0 && (
                     <p className="text-sm text-gray-400 italic">No classes today. Enjoy!</p>
                   )}
                </div>
             </div>
          </div>
        ...
        <div className="space-y-8">
           <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm h-[640px] flex flex-col">
              <h3 className="text-2xl font-black text-gray-900 mb-8">Updates</h3>
              <div className="flex-1 overflow-y-auto pr-3 space-y-8 custom-scrollbar">
                 ...
                 {recentActivity.length === 0 && (
                   <div className="h-full flex flex-col items-center justify-center text-center opacity-30">
                      ...
                      <p className="text-base font-bold">Stay tuned for updates</p>
                   </div>
                 )}
              </div>
           </div>
      ...
      {selectedAssignment && (
        <AssignmentDetailModal
          ...
          onStartSubmission={(a) => navigate(`/student/assignments/${a._id}`)}
        />
      )}
    </div>
  );
